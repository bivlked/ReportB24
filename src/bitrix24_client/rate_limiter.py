...
class AdaptiveRateLimiter:
    ...
    def update_from_response(self, response_headers: Dict[str, str], status_code: int = 200):
        """
    ...
    def _adapt_to_rate_limits(self, remaining: int, reset_time: int):
        ...
        if time_until_reset > 0 and remaining > 0:
            ...
            logger.debug(f"Adapted interval to {self._current_interval:.3f}s "
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "retry_after": max(0, self._retry_after - time.time()) if self._retry_after > 0 else 0,
