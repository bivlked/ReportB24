...
#### TASK-3.1: БАГ-5 - Удаление мёртвого кода (45 минут)
**Файлы**: `src/core/workflow.py`, `src/bitrix24_client/api_cache.py`

**Проблема**: `add_progress_callback`, `ProgressTracker`, `print_cache_report` не используются

**Подзадачи**:
- [ ] SUB-3.1.1: Поиск всех упоминаний `add_progress_callback` (grep)
- [ ] SUB-3.1.2: Проверка использования ProgressTracker
- [ ] SUB-3.1.3: Проверка использования print_cache_report
- [ ] SUB-3.1.4: Удаление неиспользуемого кода
- [ ] SUB-3.1.5: Проверка через grep на отсутствие ссылок

**Критерии удаления**:
- Код не используется ни в одном модуле
- Нет внешних зависимостей от этого кода
- Не упоминается в документации как API

#### TASK-3.2: БАГ-6 - get_requisite_details структура (1 час)
**Файл**: `src/bitrix24_client/client.py`

**Проблема**: `response.data` содержит вложенный ключ `requisite`, но код обращается к RQ_INN напрямую

**Подзадачи**:
- [ ] SUB-3.2.1: Анализ метода `get_requisite_details` (строка ~424)
- [ ] SUB-3.2.2: Логирование структуры response.data для отладки
- [ ] SUB-3.2.3: Исправление: `response.data.get("requisite", {})`
- [ ] SUB-3.2.4: Обновление метода `get_company_info_by_invoice` (строка ~486)
- [ ] SUB-3.2.5: Добавление тестов с mock данными от Bitrix24

**Решение**:
```python
# client.py - get_requisite_details
def get_requisite_details(self, requisite_id: int) -> Optional[Dict[str, Any]]:
    ...
    # 🔥 БАГ-6 FIX: response.data содержит {"requisite": {...}}
    if response.data and "requisite" in response.data:
        result = response.data["requisite"]
    else:
...
#### TASK-3.3: БАГ-7 - Кэширование пустых реквизитов (45 минут)
**Файл**: `src/bitrix24_client/api_cache.py`, `src/bitrix24_client/client.py`

**Проблема**: Кэш отказывается сохранять `None`, повторные запросы к API

**Подзадачи**:
- [ ] SUB-3.3.1: Анализ метода `cache.put()` в api_cache.py
- [ ] SUB-3.3.2: Добавление поддержки кэширования None/пустых результатов
- [ ] SUB-3.3.3: Создание sentinel значения для "отсутствующих данных"
- [ ] SUB-3.3.4: Обновление `get_requisite_details` для кэширования пустых результатов
- [ ] SUB-3.3.5: Добавление метрик для отслеживания cache hits/misses

**Решение**:
```python
# api_cache.py
CACHE_SENTINEL_EMPTY = {"__cache_empty__": True}

def put(self, method: str, params: Dict, result: Any) -> None:
    if result is None:
        result = CACHE_SENTINEL_EMPTY  # Кэшируем факт отсутствия данных
    # ... сохранение в кэш

def get(self, method: str, params: Dict) -> Any:
    result = # ... получение из кэша
    if result == CACHE_SENTINEL_EMPTY:
        return None  # Возвращаем None для пустых результатов
    return result
```
