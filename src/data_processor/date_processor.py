...
class DateProcessor:
    ...
    def parse_date(
        self, date_value: Union[str, datetime, date, int, float, None]
    ...
    def _validate_date_logic(self, dt: datetime) -> DateProcessingResult:
        ...
        if dt.year < 1900:
            return DateProcessingResult(
                is_valid=False, error_message=f"Год слишком ранний: {dt.year}"
            )

        if dt.year > current_year + 50:
            return DateProcessingResult(
                is_valid=False, error_message=f"Год слишком поздний: {dt.year}"
            )
    ...
    def parse_date_range(
    ...
    ) -> tuple[DateProcessingResult, DateProcessingResult]:
        """
        ...
        if (
            start_result.is_valid
            and end_result.is_valid
            and start_result.parsed_date > end_result.parsed_date
        ):
    ...
    def normalize_date(
        self, date_value: Union[str, datetime, date, None]
    ) -> Optional[str]:
        """
    ...
    def compare_dates(
        ...
        date2: Union[str, datetime, date, None],
    ) -> Optional[int]:
        """
        Сравнение двух дат.
