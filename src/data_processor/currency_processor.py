...
class CurrencyProcessor:
    ...
    SUPPORTED_CURRENCIES = {
        ...
        'RUR': {'symbol': '₽', 'code': 'RUB', 'name': 'Российский рубль'},  # Старое обозначение
    ...
    def __init__(self, default_currency: str = 'RUB'):
        ...
        self._compiled_patterns = [re.compile(pattern, re.IGNORECASE) for pattern in self.CURRENCY_PATTERNS]
    
    def parse_amount(self, amount_value: Union[str, int, float, Decimal, None], 
                    currency: Optional[str] = None) -> CurrencyProcessingResult:
        """
        ...
        if isinstance(amount_value, Decimal):
            return self._create_valid_result(amount_value, currency or self.default_currency, str(amount_value))
        ...
        if isinstance(amount_value, (int, float)):
            ...
            return self._create_valid_result(decimal_amount, currency or self.default_currency, str(amount_value))
    ...
    def _parse_from_string(self, amount_str: str, currency: Optional[str]) -> CurrencyProcessingResult:
        ...
        for pattern in self._compiled_patterns:
            ...
            if match:
                try:
                    ...
                    if decimal_amount > Decimal('999999999999.99'):  # Ограничение на огромные суммы
                        return CurrencyProcessingResult(
                            is_valid=False,
                            original_value=amount_str,
                    ...
                    return self._create_valid_result(decimal_amount, detected_currency, amount_str)
    ...
    def _create_valid_result(self, amount: Decimal, currency: str, original: str) -> CurrencyProcessingResult:
    ...
    def format_amount(self, amount: Decimal, currency: str = None, 
                     include_currency_symbol: bool = True) -> str:
        """
    ...
    def calculate_vat(self, amount: Union[Decimal, str, float], 
                     vat_rate: Union[str, Decimal], 
                     amount_includes_vat: bool = True) -> VATCalculationResult:
        """
        Расчёт НДС.
        ...
        if isinstance(vat_rate, str):
            ...
            if vat_rate_clean in self.VAT_RATES:
                ...
                return VATCalculationResult(
                    is_valid=False,
                    error_message=f"Неизвестная ставка НДС: {vat_rate}"
                )
        else:
        ...
        try:
            ...
            return VATCalculationResult(
                is_valid=False,
                error_message=f"Ошибка расчёта НДС: {e}"
            )
    
    def is_valid_amount(self, amount_value: Union[str, int, float, Decimal, None]) -> bool:
        """
    ...
    def normalize_amount(self, amount_value: Union[str, int, float, Decimal, None], 
                        currency: Optional[str] = None) -> Optional[str]:
        """
    ...
    def get_decimal_amount(self, amount_value: Union[str, int, float, Decimal, None]) -> Optional[Decimal]:
        """
    ...
    def compare_amounts(self, amount1: Union[str, Decimal, None], 
                       amount2: Union[str, Decimal, None]) -> Optional[int]:
        """
        Сравнение двух сумм.
    ...
    def sum_amounts(self, amounts: List[Union[str, Decimal, None]]) -> Optional[Decimal]:
        """
    ...
    def convert_currency(self, amount: Decimal, from_currency: str, 
                        to_currency: str, exchange_rate: Decimal) -> Decimal:
        """
        Конвертация валют.
