...
class BitrixConfig:
    ...
    def __post_init__(self):
        ...
        webhook_pattern = (
            r"https://[\w\-.]+(\.bitrix24\.[a-z]{2,3})?/rest/\d+/[a-zA-Z0-9_]+/?$"
        )
...
class ReportPeriodConfig:
    ...
    def __post_init__(self):
        ...
        try:
            ...
            if start_dt > end_dt:
                raise ValueError(
                    f"Дата начала ({self.start_date}) не может быть позже даты окончания ({self.end_date})"
                )
...
class ConfigReader:
    ...
    def load_config(self) -> None:
        ...
        if missing_sections:
            raise ValueError(
                f"Отсутствуют обязательные секции в config.ini: {missing_sections}"
            )
    ...
    def get_app_config(self) -> AppConfig:
        ...
        if self._app_config is None:
            ...
            save_folder = self.config.get(
                "AppSettings", "defaultsavefolder", fallback=""
            )
            filename = self.config.get("AppSettings", "defaultfilename", fallback="")

            self._app_config = AppConfig(
                default_save_folder=save_folder, default_filename=filename
            )
...
class SecureConfigReader(ConfigReader):
    ...
    def _mask_sensitive_value(self, key: str, value: str) -> str:
        ...
        if "webhook" in key.lower() and "https://" in value:
            ...
            masked = re.sub(r"(/rest/\d+/)[a-zA-Z0-9_]+/?$", r"\1***/", value)
            return masked
    ...
    def _migrate_secrets_to_env(self) -> bool:
        ...
        for env_key, value in secrets_to_migrate.items():
            ...
            key_exists = any(
                line.strip().startswith(f"{env_key}=") for line in env_content
            )
            if not key_exists:
        ...
        if migrated_keys:
            try:
                ...
                print(
                    f"⚠️  Рекомендуется удалить эти ключи из config.ini и добавить .env в .gitignore"
                )
    ...
    def get_bitrix_config(self) -> BitrixConfig:
        ...
        if self._bitrix_config is None:
            ...
            if not webhook_url:
                raise ValueError(
                    "Webhook URL не найден ни в переменных окружения, ни в .env, ни в config.ini"
                )
    ...
    def get_safe_config_info(self) -> Dict[str, Any]:
        ...
        info = {
            "sources": {
                ...
                "env_vars_count": len(
                    [k for k in os.environ.keys() if "BITRIX" in k.upper()]
                ),
        ...
        try:
            ...
            info["config"]["bitrix"] = {
                "webhook_url": self._mask_sensitive_value(
...
def create_secure_config_reader(
    config_path: str = "config.ini", env_path: str = ".env"
) -> SecureConfigReader:
    """
