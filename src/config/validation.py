...
class SystemValidator:
    ...
    def validate_python_version(self) -> ValidationResult:
        ...
        if not hasattr(sys, 'real_prefix') and not (hasattr(sys, 'base_prefix') and sys.base_prefix != sys.prefix):
            result.add_warning("Рекомендуется использовать виртуальное окружение Python")
    ...
    def validate_dependencies(self) -> ValidationResult:
        ...
        for package, min_version in required_packages.items():
            try:
                ...
                result.add_error(f"Отсутствует обязательный пакет: {package}>={min_version}")
...
class ConfigValidator:
    ...
    def validate_report_period_config(self, config: ReportPeriodConfig) -> ValidationResult:
        """
        ...
        if not result.has_errors():
            try:
                ...
                if (end_dt - start_dt) > timedelta(days=365):
                    result.add_warning("Период отчёта больше года, это может замедлить обработку")
...
class NetworkValidator:
    ...
    def validate_bitrix_connection(self, webhook_url: str, timeout: int = 10) -> ValidationResult:
        """
        ...
        try:
                ...
                try:
                    ...
                    if 'error' in data:
                        ...
                        if data['error'] == 'INVALID_GRANT':
                            result.add_error("Недействительный webhook. Проверьте корректность URL")
                        else:
                            result.add_warning(f"API ответил с ошибкой: {data.get('error_description', data['error'])}")
                    else:
            ...
            result.add_error("Не удалось подключиться к Bitrix24. Проверьте интернет-соединение")
...
class ComprehensiveValidator:
    ...
    def validate_all(self, check_network: bool = True) -> ValidationResult:
        ...
        try:
            ...
            period_result = self.config_validator.validate_report_period_config(period_config)
...
def validate_system(config_path: str = "config.ini", check_network: bool = True) -> Tuple[bool, str]:
    """
