...
class ReportLayout:
    ...
    COLUMNS = [
        ColumnDefinition(
            header="Номер", width=15.0, alignment="center", data_key="invoice_number"
        ),
        ColumnDefinition(header="ИНН", width=15.0, alignment="center", data_key="inn"),
        ColumnDefinition(
            header="Контрагент",
            width=30.0,
            alignment="left",
        ...
        ColumnDefinition(
            header="Сумма", width=18.0, alignment="right", data_key="total_amount"
        ),
        ColumnDefinition(
            header="НДС", width=18.0, alignment="center", data_key="vat_amount"
        ),
        ColumnDefinition(
            header="Дата счёта", width=15.0, alignment="right", data_key="invoice_date"
        ),
        ColumnDefinition(
        ...
        ColumnDefinition(
            header="Дата оплаты", width=15.0, alignment="right", data_key="payment_date"
        ),
    ]
...
class SummaryLayout:
    ...
    def add_summary_section(
        self, ws: Worksheet, data_row_count: int, totals: Dict[str, Any]
    ) -> None:
        """
        ...
        ws.cell(row=count_row, column=self.layout.START_COLUMN + 1).value = (
            data_row_count
        )
        ...
        if "amount_without_vat" in totals:
            ...
            ws.cell(row=amount_row, column=self.layout.START_COLUMN).value = (
                "Общая сумма без НДС:"
            )
            ws.cell(row=amount_row, column=self.layout.START_COLUMN + 4).value = totals[
                "amount_without_vat"
            ]

        if "amount_with_vat" in totals:
            total_row = summary_start_row + 2
            ws.cell(row=total_row, column=self.layout.START_COLUMN).value = (
                "Общая сумма с НДС:"
            )
            ws.cell(row=total_row, column=self.layout.START_COLUMN + 6).value = totals[
                "amount_with_vat"
            ]
...
class WorksheetBuilder:
    ...
    def create_worksheet(
        self, workbook: Workbook, sheet_name: str = "Отчёт"
    ) -> Worksheet:
        """
    ...
    def calculate_totals(self, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        """
        totals = {"count": len(data), "amount_without_vat": 0.0, "amount_with_vat": 0.0}
...
class DetailedReportLayout:
    ...
    COLUMNS = [
        ...
        ColumnDefinition(
            header="Цена", width=15.0, alignment="right", data_key="price"
        ),
        ColumnDefinition(
            header="Сумма", width=18.0, alignment="right", data_key="total_amount"
        ),
        ColumnDefinition(
            header="Сумма НДС", width=15.0, alignment="right", data_key="vat_amount"
        ),
    ]
    ...
    def setup_worksheet(self, ws: Worksheet) -> None:
        ...
        ws.row_dimensions[self.HEADER_ROW].height = (
            18  # Одинаковая высота с листом "Краткий"
        )
    ...
    def apply_zebra_effect(
        self, ws: Worksheet, data_rows: List[Dict[str, Any]]
    ) -> None:
        """
        ...
        zebra_fill = PatternFill(
            start_color=zebra_color, end_color=zebra_color, fill_type="solid"
        )
        ...
        for row_idx, row_data in enumerate(data_rows):
            ...
            if use_zebra:
                ...
                for col_idx in range(
                    self.START_COLUMN, self.START_COLUMN + self.total_columns
                ):
                    cell = ws.cell(row=excel_row, column=col_idx)
                    cell.fill = zebra_fill

    def apply_invoice_separator_borders(
        self, ws: Worksheet, data_rows: List[Dict[str, Any]]
    ) -> None:
        """
        ...
        for row_idx, row_data in enumerate(data_rows):
            ...
            if invoice_id != current_invoice_id and last_invoice_row is not None:
                self._apply_separator_border_to_row(
                    ws, last_invoice_row, separator_border
                )
    ...
    def _apply_separator_border_to_row(
        self, ws: "Worksheet", row_idx: int, border
    ) -> None:
        """
...
class DetailedWorksheetBuilder:
    ...
    def create_detailed_worksheet(
        self, workbook: Workbook, sheet_name: str = "Полный"
    ) -> Worksheet:
        """
    ...
    def write_detailed_data(
        ...
        for row_idx, row_data in enumerate(data_rows):
            for col_idx, col_def in enumerate(self.layout.COLUMNS):
                excel_row, excel_col = self.layout.get_data_cell_position(
                    row_idx, col_idx
                )
                ...
                if col_idx == 7 and str(cell.value).lower() == "нет":
                    ...
                    cell.alignment = Alignment(
                        horizontal=col_def.alignment, vertical="center"
                    )
                    # 🔧 УНИФИКАЦИЯ: Применяем числовое форматирование как в кратком отчете
                    cell.number_format = self._get_detailed_column_number_format(
                        col_idx
                    )
    ...
    def _apply_detailed_table_borders(self, ws: Worksheet, data_rows: int) -> None:
        ...
        last_col = (
            self.layout.START_COLUMN + self.layout.total_columns - 1
        )  # последний столбец
        ...
        for row in range(self.layout.HEADER_ROW, last_data_row + 1):
            for col in range(self.layout.START_COLUMN, last_col + 1):
                ...
                border_left = (
                    thick_border
                    if col == self.layout.START_COLUMN
                    else cell.border.left
                )
                border_right = thick_border if col == last_col else cell.border.right
                border_top = (
                    thick_border if row == self.layout.HEADER_ROW else cell.border.top
                )
                border_bottom = (
    ...
    def add_detailed_summary(
        self, ws: Worksheet, data_row_count: int, summary_stats: Dict[str, Any]
    ) -> None:
        """
        ...
        # Products count
        ws.cell(row=summary_start_row, column=self.layout.START_COLUMN).value = (
            "Всего товаров:"
        )
        ws.cell(row=summary_start_row, column=self.layout.START_COLUMN + 1).value = (
            data_row_count
        )
        ...
        if "total_invoices" in summary_stats:
            ...
            ws.cell(row=invoices_row, column=self.layout.START_COLUMN).value = (
                "Всего счетов:"
            )
            ws.cell(row=invoices_row, column=self.layout.START_COLUMN + 1).value = (
                summary_stats["total_invoices"]
            )

    def _adjust_detailed_column_widths(
        self, ws: Worksheet, data_rows: List[Dict[str, Any]]
    ) -> None:
        """
