...
Создаёт комплексный Excel отчёт с двумя листами:
...
Отображает прогресс выполнения и детальную статистику результатов.
"""
            ...
            try:
                ...
                for i, invoice in enumerate(invoices, 1):
                    ...
                    if products_result.get("has_error"):
                        ...
                        failed_invoices.append(
                            {
                                "id": invoice_id,
                                "account_number": invoice.get(
                                    "accountNumber", f"Счет #{invoice_id}"
                                ),
                                "error": error_msg,
                            }
                        )
                ...
                if failed_invoices:
                    ...
                    ConsoleUI.print_warning(
                        f"⚠️  {len(failed_invoices)} счетов имели ошибки:"
                    )
