...
class INNProcessor:
    ...
    INDIVIDUAL_COEFFICIENTS_11 = [
        ...
        6,
        8,
    ]  # Для 11-й цифры 12-цифрового ИНН
    INDIVIDUAL_COEFFICIENTS_12 = [
        3,
        7,
        2,
        4,
        ...
        6,
        8,
    ]  # Для 12-й цифры 12-цифрового ИНН
    ...
    def validate_inn(self, inn: Union[str, int, None]) -> INNValidationResult:
        ...
        if inn is None or inn == "":
            return INNValidationResult(
                is_valid=False, error_message="ИНН не может быть пустым"
            )
        ...
        if not clean_inn:
            return INNValidationResult(
                is_valid=False, error_message="ИНН должен содержать только цифры"
            )
    ...
    def _validate_legal_inn(self, inn: str) -> INNValidationResult:
        ...
        try:
            ...
            control_sum = sum(
                digit * coeff
                for digit, coeff in zip(digits[:9], self.LEGAL_COEFFICIENTS)
            )
            ...
            return INNValidationResult(
                is_valid=False, error_message=f"Ошибка при валидации ИНН: {e}"
            )
    ...
    def _validate_individual_inn(self, inn: str) -> INNValidationResult:
        ...
        try:
            ...
            control_sum_11 = sum(
                digit * coeff
                for digit, coeff in zip(digits[:10], self.INDIVIDUAL_COEFFICIENTS_11)
            )
            ...
            control_sum_12 = sum(
                digit * coeff
                for digit, coeff in zip(digits[:11], self.INDIVIDUAL_COEFFICIENTS_12)
            )
            ...
            if (
                expected_check_digit_11 == actual_check_digit_11
                and expected_check_digit_12 == actual_check_digit_12
            ):
